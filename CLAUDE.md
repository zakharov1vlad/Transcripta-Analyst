...
- В 00:00 МСК бот показывает **итог за прошлый день** (не нули)
...
- [x] Часовой бот v3:
  - новые/старые юзеры, первичные/повторные/минуты, ожидаемые/факт списания, DAU старые
  - оценки из `reviews.rating` (не surveys)
  - раздел «Контакты» удалён
  - в 00:00 показывает итог за прошлый день
  - ожидаемые списания фиксируются на начало дня (файловый кэш `data/renewals_cache.json`)
  - все метрики-функции принимают опциональный `date_str` для запроса данных за произвольную дату
