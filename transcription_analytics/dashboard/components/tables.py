...
def get_daily_summary(days: int = 30) -> pd.DataFrame:
    ...
    df = fetch_df(f"""
        ...
        FROM (
            SELECT DATE_SUB(DATE(CONVERT_TZ(NOW(), '+00:00', '+03:00')), INTERVAL n DAY) AS date
            ...
            WHERE DATE_SUB(DATE(CONVERT_TZ(NOW(), '+00:00', '+03:00')), INTERVAL n DAY) >= DATE_SUB(DATE(CONVERT_TZ(NOW(), '+00:00', '+03:00')), INTERVAL {days} DAY)
        ) d
        LEFT JOIN (
            SELECT DATE(CONVERT_TZ(created_at, '+00:00', '+03:00')) AS date, COUNT(*) AS new_users
            FROM users WHERE COALESCE(is_test, '0') != '1'
            GROUP BY DATE(CONVERT_TZ(created_at, '+00:00', '+03:00'))
        ) u ON u.date = d.date
        LEFT JOIN (
            SELECT DATE(CONVERT_TZ(tr.created_at, '+00:00', '+03:00')) AS date,
            ...
            GROUP BY DATE(CONVERT_TZ(tr.created_at, '+00:00', '+03:00'))
        ) t ON t.date = d.date
        LEFT JOIN (
            SELECT DATE(CONVERT_TZ(ph.created_at, '+00:00', '+03:00')) AS date,
